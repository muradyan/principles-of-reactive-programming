...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = fs match {
      case Nil => Future(Nil)
      case (head::tail) => head.flatMap(t => all(tail).flatMap(ts => Future(t :: ts)))
    }
     ...
     */
    def delay(t: Duration): Future[Unit] = Future { Try (blocking { Await.result(Future.never, t) }) }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = Try( blocking {Await.result(f, 0 seconds)} ) match {
      case Success(v) => v
     ...
     */
    def continueWith[S](cont: Future[T] => S): Future[S] = f map { v => cont(f) }
     ...
     *  The resulting future contains a value returned by `cont`.
     */
    def continue[S](cont: Try[T] => S): Future[S] = f map { v => cont(Try(v)) }
